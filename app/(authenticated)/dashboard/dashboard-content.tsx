...
async function fetchDashboardData() {
  ...
  const lowStock = [
    ...products.map((p) => ({
      name: p.name,
      qty: p.quantityOnHand || p.quantity_on_hand || 0,
      reorder: p.reorderLevel || p.reorder_level || 0,
      kind: "Product",
    })),
    ...ingredients.map((i) => ({
      name: i.name,
      qty: i.currentStock || i.current_stock || 0,
      reorder: i.reorderLevel || i.reorder_level || 0,
      kind: "Ingredient",
    })),
  ]
    .filter((item) => item.reorder > 0 && item.qty <= item.reorder)
    .slice(0, 8);
